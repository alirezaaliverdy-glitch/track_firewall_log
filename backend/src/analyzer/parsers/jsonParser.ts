...
export function parseJsonLog(text: string): ImportParseResult {
  ...
  const rows = values.map(objectToRow).filter((row): row is RawLogRow => row !== undefined);
...
export function parseNdjsonLog(text: string): ImportParseResult {
  ...
  lines.forEach((line, index) => {
    ...
    try {
      ...
      else if (warnings.length < 20) warnings.push({ line: index + 1, message: "NDJSON line was valid JSON but not an object." });
    } catch {
      if (warnings.length < 20) warnings.push({ line: index + 1, message: "Invalid JSON line skipped." });
    }
  });
