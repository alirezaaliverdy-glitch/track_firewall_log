import type { FirewallTypeSelection, LogCapability, LogProfile, NormalizedLog, RawLogRow, VendorType } from "./types.js";
...
function rawText(rawRows: RawLogRow[]): string {
  return rawRows.slice(0, 50).map((row) => Object.values(row).join(" ")).join(" ").toLowerCase();
}

function scoreVendor(keys: string[], text: string, vendor: VendorType, indicators: string[], patterns: RegExp[] = []) {
  const matches = indicators.filter((indicator) => keys.includes(indicator.toLowerCase()));
  const patternMatches = patterns.filter((pattern) => pattern.test(text)).map((pattern) => pattern.source);
  return { vendor, score: matches.length + patternMatches.length, matches: [...matches, ...patternMatches] };
}
...
export function detectVendorFromFields(rawRows: RawLogRow[], normalizedLogs: NormalizedLog[]): VendorType {
  ...
  const scores = [
    scoreVendor(keys, text, "fortigate", ["policyid", "policyname", "logid", "subtype", "vd", "srcip", "dstip", "dstport", "sentbyte", "rcvdbyte", "action", "service", "appcat"]),
    scoreVendor(keys, text, "mikrotik", ["topics", "firewall", "chain", "in-interface", "out-interface", "src-address", "dst-address", "src-port", "dst-port", "protocol", "action", "connection-state"], [/firewall,info/, /\bforward:/]),
    scoreVendor(keys, text, "pfsense", ["filterlog", "rule", "tracker", "interface", "reason", "act", "direction", "ipversion", "protocol", "src", "dst", "srcport", "dstport"]),
    scoreVendor(keys, text, "paloalto", ["subtype", "receive_time", "serial", "type", "threat_content_type", "src", "dst", "rule", "app", "action"]),
    scoreVendor(keys, text, "cisco-asa", [], [/asa-/, /%asa/, /access-list/, /built inbound/, /built outbound/, /\bdeny\b/])
  ].sort((a, b) => b.score - a.score);

  const best = scores[0];
  if (best && best.score >= 3) return best.vendor;
...
export function calculateFieldCoverage(normalizedLogs: NormalizedLog[]): Record<string, number> {
  const fields = ["timestamp", "action", "srcIp", "dstIp", "srcPort", "dstPort", "protocol", "bytes", "packets", "service", "application", "user", "policyId", "policyName", "ruleName", "ruleDisplayName", "natSrcPort", "natDstPort"];
...
function inferCapabilities(profile: LogProfile): Pick<LogProfile, "capabilities" | "disabledCapabilities"> {
  const has = (field: string) => (profile.fieldCoverage[field] ?? 0) > 0;
  const hasUsefulPolicy = has("policyId") || has("policyName") || has("ruleName") || (profile.fieldCoverage.ruleDisplayName ?? 0) > 0.05;
  ...
  enable("geoAnalysis", false, "Geo analysis is not available because no geo-enrichment is performed server-side.");
...
export function buildLogProfile(rawRows: RawLogRow[], normalizedLogs: NormalizedLog[], selectedVendor: FirewallTypeSelection): LogProfile {
  ...
  const availableFields = Array.from(new Set([...Object.keys(fieldCoverage).filter((field) => fieldCoverage[field] > 0), ...rawKeys(rawRows)]));
  ...
  const baseProfile: LogProfile = {
    ...
    confidence: Math.min(0.99, Math.max(0.35, availableFields.length / 20 + (detectedVendor !== "generic" ? 0.35 : 0))),
    availableFields,
    missingFields,
    capabilities: [],
  ...
  return {
    ...
    reasons: [
      ...
      missingFields.length > 0 ? `Missing fields: ${missingFields.join(", ")}.` : "Core analysis fields are present."
    ]
  };
}
