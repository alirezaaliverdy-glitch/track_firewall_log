...
export const assetRoutes: FastifyPluginAsync = async (app) => {
  async function activeCompanyId(userId?: string, requested?: unknown) {
    const requestedId = typeof requested === "string" ? requested : "";
    const company = await prisma.company.findFirst({ where: { ...(requestedId ? { id: requestedId } : {}), ...(userId ? { ownerId: userId } : {}), deletedAt: null }, orderBy: { createdAt: "asc" }, select: { id: true } });
    if (!company) throw new Error("ACTIVE_COMPANY_REQUIRED");
    return company.id;
  }
  ...
  app.delete<{ Params: { id: string } }>("/api/assets/:id", async (request, reply) => {
    try {
      const result = await removeAssetFromInventory(request.params.id, request.authUser?.id);
