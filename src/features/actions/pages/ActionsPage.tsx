  ...
  return (
    <section className="page-stack">
      <PageHeader
        ...
        actions={<div className="flex flex-wrap gap-2"><button className="primary-button" type="button" onClick={() => { setCatalogOpen(false); document.querySelector('.operator-run-card')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }}>{isFa ? "عملیات جدید" : "New Action"}</button><button className="secondary-button" type="button" onClick={() => setCatalogOpen(!catalogOpen)}>{isFa ? "کتابخانه کامل" : "Full action library"}</button></div>}
      />
