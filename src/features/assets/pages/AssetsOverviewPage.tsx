...
export default function AssetsOverviewPage() {
  ...
  return (
    <section className="page-stack">
      <PageHeader title="دارایی ها" eyebrow="دید عملیاتی دارایی" description="نمای فشرده دارایی های شناخته شده، وضعیت سلامت و آخرین همگام سازی." actions={<Link className="primary-link" to="/assets/devices/new">ثبت دستگاه</Link>} />
      ...
      <div className="content-grid">
        ...
        <section className="content-panel">
          ...
          {assets.filter((asset) => !["online", "healthy"].includes(asset.healthState)).slice(0, 5).map((asset) => (
            <Link key={asset.id} to={`/assets/devices/${asset.device?.id ?? asset.id}`} className="list-row">{asset.name}<span>{asset.healthState}</span></Link>
          ))}
        ...
        <section className="content-panel">
          ...
          <Link to="/assets/sync" className="primary-link">رفتن به همگام سازی</Link>
        </section>
      </div>
    </section>
  );
