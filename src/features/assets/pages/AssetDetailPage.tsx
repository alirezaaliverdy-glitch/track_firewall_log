...
export default function AssetDetailPage({ params }: RouteComponentProps) {
  ...
  return (
    <section className="page-stack">
      ...
      <nav className="workspace-tabs" aria-label="بخش‌های فضای کاری">{sections.map(([key, label]) => <a key={key} aria-current={section === key ? "page" : undefined} href={`/assets/devices/${deviceId}/${key}`}>{label}</a>)}{workspace.vendor.sections.map((item) => <a key={`vendor-${item.key}`} aria-current={section === item.key ? "page" : undefined} href={`/assets/devices/${deviceId}/${item.key}`}>{isFa ? item.titleFa : item.titleEn}</a>)}</nav>
      {content}
    </section>
  );
}
